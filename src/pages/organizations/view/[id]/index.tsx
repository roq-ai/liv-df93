...
import { deleteModeratorById, createModerator } from 'apiSdk/moderators';
...
function OrganizationViewPage() {
  ...
  const moderatorHandleCreate = async () => {
    setCreateError(null);
    try {
      await createModerator({ organization_id: id, user_id: moderatorUserId });
  ...
  const moderatorHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteModeratorById(id);
      await mutate();
    } catch (error) {
            ...
            <Box>
              <Stack spacing={2} mb={8}>
                ...
                <Flex gap={5} alignItems="flex-end">
                  <Box flex={1}>
                    <UserSelect name={'moderator_user'} value={moderatorUserId} handleChange={setModeratorUserId} />
                  </Box>
                  <Button
                    colorScheme="blue"
                    mt="4"
                    mr="4"
                    onClick={moderatorHandleCreate}
                    isDisabled={!moderatorUserId}
                  >
                    Create
                  </Button>
                </Flex>
                          ...
                          <Td>
                            {hasAccess('user', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                              <IconButton
                                onClick={(e) => {
                                  e.stopPropagation();
                                  moderatorHandleDelete(record.id);
                                }}
                                colorScheme="red"
                                variant="outline"
                                aria-label="edit"
